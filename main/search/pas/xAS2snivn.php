...
if(!empty($json['suggestionGroups'][0]['searchSuggestions'][0]['displayText'])){
    ...
    for ($x = 0; $x < count($json['suggestionGroups'][0]['searchSuggestions']); $x++) {
        ...
        echo('<li id="sb-'.$x.'" class="display_box sb-link sb-content" onclick="location.href=\'images?q='.$obj2.'\'"><img class="default-o" src="src/public/icon-sea-bl.png"><img class="dark-o" src="src/public/icon-sea-w.png"><a class="an-link"><span>'.$obj.'</span></a></li>');
      }
    echo('</ul>');
};
?>
